...
docker exec vanchor-kicad kicad-cli pcb export pos --units mm --side both \
  -o $FAB/vanchor-helm.pos $HW/vanchor-helm.kicad_pcb
...
docker exec vanchor-kicad kicad-cli sch export bom \
  ...
  -o $FAB/vanchor-helm-bom.csv $HW/vanchor-helm.kicad_sch
docker exec vanchor-kicad kicad-cli sch export pdf \
  -o $FAB/vanchor-helm-schematic.pdf $HW/vanchor-helm.kicad_sch

# Renders
docker exec vanchor-kicad kicad-cli pcb render --side top --background opaque \
  -o $FAB/renders/board-top.png $HW/vanchor-helm.kicad_pcb
docker exec vanchor-kicad kicad-cli pcb render --side bottom --background opaque \
  -o $FAB/renders/board-bottom.png $HW/vanchor-helm.kicad_pcb
docker exec vanchor-kicad kicad-cli pcb render --perspective --rotate "-30,20,-20" \
  --zoom 0.9 --background opaque \
  -o $FAB/renders/board-3d.png $HW/vanchor-helm.kicad_pcb

# Zip the gerbers for upload
docker exec vanchor-kicad sh -c "cd $FAB/gerbers && rm -f ../vanchor-helm-gerbers.zip && zip -q ../vanchor-helm-gerbers.zip *"
echo "fab outputs in hardware/fab/"
